...
const getRequestResponses = (responses = {}) => {
  if (!responses || !responses['200']) {
    ...
    if (originalRef) {
      let type = originalRef.substring(0, originalRef.length - 1).replaceAll('Result«', '').replaceAll('«', '<SwaggerDTO.').replaceAll('»', '>').replaceAll('List', 'Array');
      if (type === 'Void') {
        type = 'void';
      }
      if (type === 'int') {
      ...
      return `SwaggerDTO.ResponseResult<${type}>`;
    } else {
      return '';
    }
  }
  ...
  for (let router in paths) {
    ...
    if (result.tags.includes(tag)) {
      ...
      export const ${result.operationId} = (params${getRequestParameters(result.parameters)}, config?: AxiosRequestConfig) => {
        return new Promise((resolve: ${getRequestResponses(result.responses)}) => {
